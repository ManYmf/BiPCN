...
class discPC:
    def __init__(self, input_dim, output_dim, activation=sigmoid):
        ...
        # 添加偏置项
        self.b1 = np.zeros((output_dim, 1))
    
    def initialize_activity(self, x1):
        """前馈扫描初始化高层活动（输出层x2）"""
        # 初始化公式：x2^(0) = f(V1 @ x1 + b1)
        x2_initial = self.activation(self.V1.T @ x1 + self.b1)
        return x2_initial
    ...
    def update_activity(self, x1, x2, target=None, lr=0.1, m=20, lambda_target=0.9):
        ...
        for _ in range(m):
            # 自下而上预测值：f(V1 @ x1 + b1)
            pred = self.activation(self.V1.T @ x1 + self.b1)
            # 预测误差
            epsilon = x2 - pred
    ...
    def update_weight(self, x1, x2, target, lr=0.01):
        """Hebbian规则更新权重V1和偏置b1"""
        # 计算预测值
        pred = self.activation(self.V1.T @ x1 + self.b1)
        
        # 后突触误差：使用目标与预测之间的误差，更符合监督学习
        error = target - pred
        
        # 前突触活动：使用输入层活动
        pre_synaptic = x1
        
        # Hebbian更新：权重变化与前突触活动和误差成正比
...
if __name__ == "__main__":
    # 超参数
    task = 'OR'  # 可切换为'OR'
    n_epochs = 2000  # 增加训练轮数
    m = 50  # 增加活动更新次数
    lr_activity = 0.1  # 神经活动学习率
    lr_weight = 0.1  # 增加权重学习率
    lambda_target = 0.9  # 目标约束权重
    ...
    # 初始化网络
    model = discPC(input_dim, output_dim)
